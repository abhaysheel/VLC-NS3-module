...
const uint32_t VlcPhy::aMaxPhyPacketSize = 200; // max PSDU in octets
const uint32_t VlcPhy::aTurnaroundTime_TX_RX = 0;  // RX-to-TX or TX-to-RX in symbol periods
const uint32_t VlcPhy::aTurnaroundTime_RX_TX = 5120;

// IEEE802.15.4-2006 Table 2 in section 6.1.2 (kb/s and ksymbol/s)
// The index follows VlcPhyOption
const VlcPhyDataAndSymbolRates
VlcPhy::dataSymbolRates[2] = { { 1250.0, 1250.0},
                                  { 6000.0, 6000.0}};
// IEEE802.15.4-2006 Table 19 and Table 20 in section 6.3.
// The PHR is 1 octet and it follows phySymbolsPerOctet in Table 23
// The index follows VlcPhyOption
// TODO : CHECK
const VlcPhyPpduHeaderSymbolNumber
VlcPhy::ppduHeaderSymbolNumbers[2] = { { 124.0, 32.0, 16.0 },
                                          { 124.0, 32.0, 16.0}};
...
VlcPhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::VlcPhy")
    ...
    .SetGroupName ("Vlc")
    .AddConstructor<VlcPhy> ()
    .AddTraceSource ("TrxStateValue",
                     "The state of the transceiver",
                     MakeTraceSourceAccessor (&VlcPhy::m_trxState),
                     "ns3::TracedValueCallback::VlcPhyEnumeration")
    .AddTraceSource ("TrxState",
                     "The state of the transceiver",
                     MakeTraceSourceAccessor (&VlcPhy::m_trxStateLogger),
                     "ns3::VlcPhy::StateTracedCallback")
...
VlcPhy::VlcPhy (void)
  ...
  //  m_setTRXState ()
{
  m_trxState = IEEE_802_15_7_PHY_TRX_OFF;
  m_trxStatePending = IEEE_802_15_7_PHY_IDLE;
  ...
  ChangeTrxState (IEEE_802_15_7_PHY_TRX_OFF);
}
...
VlcPhy::DoDispose (void)
{
  ...
  m_pdDataConfirmCallback = MakeNullCallback< void, VlcPhyEnumeration > ();
  m_plmeCcaConfirmCallback = MakeNullCallback< void, VlcPhyEnumeration > ();
  // TODO : CHECK
  //m_plmeEdConfirmCallback = MakeNullCallback< void, VlcPhyEnumeration,uint8_t > ();
  m_plmeGetAttributeConfirmCallback = MakeNullCallback< void, VlcPhyEnumeration, VlcPibAttributeIdentifier, VlcPhyPibAttributes* > ();
  m_plmeSetTRXStateConfirmCallback = MakeNullCallback< void, VlcPhyEnumeration > ();
  m_plmeSetAttributeConfirmCallback = MakeNullCallback< void, VlcPhyEnumeration, VlcPibAttributeIdentifier > ();

  SpectrumPhy::DoDispose ();
}

Ptr<NetDevice>
VlcPhy::GetDevice (void) const
{
  NS_LOG_FUNCTION (this);
  return m_device;
}
...
void
VlcPhy::SetDevice (Ptr<NetDevice> d)
{
...
void
VlcPhy::SetMobility (Ptr<MobilityModel> m)
{
...
void
VlcPhy::SetChannel (Ptr<SpectrumChannel> c)
{
...
VlcPhy::GetRxSpectrumModel (void) const
{
  NS_LOG_FUNCTION (this);
  if (m_txPsd)
    {
...
void
VlcPhy::SetAntenna (Ptr<AntennaModel> a)
{
...
void
VlcPhy::StartRx (Ptr<SpectrumSignalParameters> spectrumRxParams)
{
  ...
  Ptr<VlcSpectrumSignalParameters> vlcRxParams = DynamicCast<VlcSpectrumSignalParameters> (spectrumRxParams);
  ...
  if (vlcRxParams == 0)
    {
      ...
      if (!m_ccaRequest.IsExpired ())
        {
          double power = VlcSpectrumValueHelper::TotalAvgPower (m_signal->GetSignalPsd (), m_phyPIBAttributes.phyCurrentChannel);
      ...
      Simulator::Schedule (spectrumRxParams->duration, &VlcPhy::EndRx, this, spectrumRxParams);
      return;
    }

  Ptr<Packet> p = (vlcRxParams->packetBurst->GetPackets ()).front ();
  NS_ASSERT (p != 0);

  // Prevent PHY from receiving another packet while switching the transceiver state.
  if (m_trxState == IEEE_802_15_7_PHY_RX_ON && !m_setTRXState.IsRunning ())
    {
      ...
      // SINR.
      NS_LOG_DEBUG (this << " receiving packet with power: " << 10 * log10(VlcSpectrumValueHelper::TotalAvgPower (vlcRxParams->psd, m_phyPIBAttributes.phyCurrentChannel)) + 30 << "dBm");
      m_signal->AddSignal (vlcRxParams->psd);
      Ptr<SpectrumValue> interferenceAndNoise = m_signal->GetSignalPsd ();
      *interferenceAndNoise -= *vlcRxParams->psd;
    //  *interferenceAndNoise += *m_noise;
      double sinr = VlcSpectrumValueHelper::TotalAvgPower (vlcRxParams->psd, m_phyPIBAttributes.phyCurrentChannel) / VlcSpectrumValueHelper::TotalAvgPower (interferenceAndNoise, m_phyPIBAttributes.phyCurrentChannel);
      ...
      if (10 * log10 (sinr) > -5)
        {
          ChangeTrxState (IEEE_802_15_7_PHY_BUSY_RX);
          m_currentRxPacket = std::make_pair (vlcRxParams, false);
          m_phyRxBeginTrace (p);
  ...
  else if (m_trxState == IEEE_802_15_7_PHY_BUSY_RX)
    {
      // Drop the new packet.
...
void
VlcPhy::CheckInterference (void)
{
  // Calculate whether packet was lost.
  VlcSpectrumValueHelper psdHelper;
  Ptr<VlcSpectrumSignalParameters> currentRxParams = m_currentRxPacket.first;

  // We are currently receiving a packet.
  if (m_trxState == IEEE_802_15_7_PHY_BUSY_RX)
    {
      ...
      if (m_errorModel != 0)
        {
          ...
          *interferenceAndNoise -= *currentRxParams->psd;
          //*interferenceAndNoise += *m_noise;
          double sinr = VlcSpectrumValueHelper::TotalAvgPower (currentRxParams->psd, m_phyPIBAttributes.phyCurrentChannel) / VlcSpectrumValueHelper::TotalAvgPower (interferenceAndNoise, m_phyPIBAttributes.phyCurrentChannel);
          ...
          VlcWqiTag tag (std::numeric_limits<uint8_t>::max ());
...
void
VlcPhy::EndRx (Ptr<SpectrumSignalParameters> par)
{
  NS_LOG_FUNCTION (this);

  Ptr<VlcSpectrumSignalParameters> params = DynamicCast<VlcSpectrumSignalParameters> (par);
          ...
          if (!m_setTRXState.IsRunning ())
            {
              ...
              if (!m_plmeSetTRXStateConfirmCallback.IsNull ())
                {
                  m_plmeSetTRXStateConfirmCallback (IEEE_802_15_7_PHY_SUCCESS);
                }
            }
        }
      else
        {
          ChangeTrxState (IEEE_802_15_7_PHY_RX_ON);
        }
    }
}

void
VlcPhy::PdDataRequest (const uint32_t psduLength, Ptr<Packet> p)
{
  ...
  if (psduLength > aMaxPhyPacketSize)
    {
      if (!m_pdDataConfirmCallback.IsNull ())
        {
          m_pdDataConfirmCallback (IEEE_802_15_7_PHY_UNSPECIFIED);
        }
  ...
  if (!m_setTRXState.IsRunning ())
    {
      if (m_trxState == IEEE_802_15_7_PHY_TX_ON)
        {
          //send down
          NS_ASSERT (m_channel);
          ...
          VlcWqiTag wqiTag;
          p->RemovePacketTag (wqiTag);

          m_phyTxBeginTrace (p);
          m_currentTxPacket.first = p;
          m_currentTxPacket.second = false;

          Ptr<VlcSpectrumSignalParameters> txParams = Create<VlcSpectrumSignalParameters> ();
          ...
          m_pdDataRequest = Simulator::Schedule (txParams->duration, &VlcPhy::EndTx, this);
          ChangeTrxState (IEEE_802_15_7_PHY_BUSY_TX);
          return;
        }
      else if ((m_trxState == IEEE_802_15_7_PHY_RX_ON)
               || (m_trxState == IEEE_802_15_7_PHY_TRX_OFF)
               || (m_trxState == IEEE_802_15_7_PHY_BUSY_TX) )
        {
          if (!m_pdDataConfirmCallback.IsNull ())
            {
...
void
VlcPhy::PlmeCcaRequest (void)
{
  NS_LOG_FUNCTION (this);

  if (m_trxState == IEEE_802_15_7_PHY_RX_ON || m_trxState == IEEE_802_15_7_PHY_BUSY_RX)
    {
      m_ccaPeakPower = 0.0;
      Time ccaTime = Seconds (8.0 / GetDataOrSymbolRate (false));
      m_ccaRequest = Simulator::Schedule (ccaTime, &VlcPhy::EndCca, this);
    }
  else
    {
      if (!m_plmeCcaConfirmCallback.IsNull ())
        {
          if (m_trxState == IEEE_802_15_7_PHY_TRX_OFF)
            {
              m_plmeCcaConfirmCallback (IEEE_802_15_7_PHY_TRX_OFF);
            }
          else
            {
              m_plmeCcaConfirmCallback (IEEE_802_15_7_PHY_BUSY);
            }
        }
    }
}
...
void
VlcPhy::PlmeGetAttributeRequest (VlcPibAttributeIdentifier id)
{
  NS_LOG_FUNCTION (this << id);
  VlcPhyEnumeration status;
  ...
  switch (id)
    ...
    case phyCCAMode:
    case phyDim:
    case phyUseExtendedMode:
    case phyColorFunction:
    case phyBlinkingNotificationFrequency:
    //case phyCurrentPage:
    //case phyMaxFrameDuration:
    //case phySHRDuration:
    //case phySymbolsPerOctet:
      {
        status = IEEE_802_15_7_PHY_SUCCESS;
        break;
      }
    default:
      {
        status = IEEE_802_15_7_PHY_UNSUPPORTED_ATTRIBUTE;
        break;
      }
    }
  if (!m_plmeGetAttributeConfirmCallback.IsNull ())
    {
      VlcPhyPibAttributes retValue;
      memcpy (&retValue, &m_phyPIBAttributes, sizeof(VlcPhyPibAttributes));
...
// Section 6.2.2.7.3
void
VlcPhy::PlmeSetTRXStateRequest (VlcPhyEnumeration state)
{
  NS_LOG_FUNCTION (this << state);

  // Check valid states (Table 14)
  NS_ABORT_IF ( (state != IEEE_802_15_7_PHY_RX_ON)
                && (state != IEEE_802_15_7_PHY_TRX_OFF)
                && (state != IEEE_802_15_7_PHY_FORCE_TRX_OFF)
                && (state != IEEE_802_15_7_PHY_TX_ON) );
  ...
  if ( ((state == IEEE_802_15_7_PHY_RX_ON)
        || (state == IEEE_802_15_7_PHY_TRX_OFF))
       && (m_trxState == IEEE_802_15_7_PHY_BUSY_TX) )
    {
  ...
  if (state == IEEE_802_15_7_PHY_TRX_OFF)
    {
      //CancelEd (state);
      ...
      else if (m_trxState == IEEE_802_15_7_PHY_RX_ON || m_trxState == IEEE_802_15_7_PHY_TX_ON)
        {
          ChangeTrxState (IEEE_802_15_7_PHY_TRX_OFF);
          if (!m_plmeSetTRXStateConfirmCallback.IsNull ())
            {
  ...
  if (state == IEEE_802_15_7_PHY_TX_ON)
    {
      //CancelEd (state);

      NS_LOG_DEBUG ("turn on PHY_TX_ON");
      if ((m_trxState == IEEE_802_15_7_PHY_BUSY_RX) || (m_trxState == IEEE_802_15_7_PHY_RX_ON))
        {
          ...
          m_trxStatePending = IEEE_802_15_7_PHY_TX_ON;
          ...
          Time setTime = Seconds ( (double) aTurnaroundTime_RX_TX / GetDataOrSymbolRate (false));
          m_setTRXState = Simulator::Schedule (setTime, &VlcPhy::EndSetTRXState, this);
          return;
        }
      else if (m_trxState == IEEE_802_15_7_PHY_BUSY_TX || m_trxState == IEEE_802_15_7_PHY_TX_ON)
        {
          // We do NOT change the transceiver state here. We only report that
          // the transceiver is already in TX_ON state.
          if (!m_plmeSetTRXStateConfirmCallback.IsNull ())
            {
              m_plmeSetTRXStateConfirmCallback (IEEE_802_15_7_PHY_TX_ON);
            }
          return;
        }
      else if (m_trxState == IEEE_802_15_7_PHY_TRX_OFF)
        {
          ChangeTrxState (IEEE_802_15_7_PHY_TX_ON);
          if (!m_plmeSetTRXStateConfirmCallback.IsNull ())
            {
              m_plmeSetTRXStateConfirmCallback (IEEE_802_15_7_PHY_TX_ON);
            }
  ...
  if (state == IEEE_802_15_7_PHY_FORCE_TRX_OFF)
    {
      if (m_trxState == IEEE_802_15_7_PHY_TRX_OFF)
        {
      ...
      else
        {
          ...
          if (m_trxState == IEEE_802_15_7_PHY_BUSY_TX)
            {
  ...
  if (state == IEEE_802_15_7_PHY_RX_ON)
    {
      if (m_trxState == IEEE_802_15_7_PHY_TX_ON || m_trxState == IEEE_802_15_7_PHY_TRX_OFF)
        {
          // Turnaround delay
          // TODO: Does it really take aTurnaroundTime to switch the transceiver state,
          //       even when the transmitter is not busy? (6.9.1)
          m_trxStatePending = IEEE_802_15_7_PHY_RX_ON;
      ...
      else if (m_trxState == IEEE_802_15_7_PHY_BUSY_RX)
        {
          if (!m_plmeSetTRXStateConfirmCallback.IsNull ())
            {
              m_plmeSetTRXStateConfirmCallback (IEEE_802_15_7_PHY_RX_ON);
            }
          return;
        }
    }
...
bool
VlcPhy::ChannelSupported (uint8_t channel)
{
...
void
VlcPhy::PlmeSetAttributeRequest (VlcPibAttributeIdentifier id,
                                 VlcPhyPibAttributes* attribute)
{
  NS_LOG_FUNCTION (this << id << attribute);
  NS_ASSERT (attribute);
  VlcPhyEnumeration status = IEEE_802_15_7_PHY_SUCCESS;
    ...
    case phyCurrentChannel:
      {
        if (!ChannelSupported (attribute->phyCurrentChannel))
          {
            status = IEEE_802_15_7_PHY_INVALID_PARAMETER;
          }
            ...
            if (m_trxStatePending != IEEE_802_15_7_PHY_IDLE)
              {
                ...
                if (!m_plmeSetTRXStateConfirmCallback.IsNull ())
                  {
                    m_plmeSetTRXStateConfirmCallback (IEEE_802_15_7_PHY_TRX_OFF);
                  }
              }
            ...
            if (PhyIsBusy ())
              {
                ...
                if (!m_pdDataConfirmCallback.IsNull ())
                  {
                    m_pdDataConfirmCallback (IEEE_802_15_7_PHY_TRX_OFF);
                  }
              }
...
void
VlcPhy::SetPdDataIndicationCallback (PdDataIndicationCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
VlcPhy::SetPdDataConfirmCallback (PdDataConfirmCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
VlcPhy::SetPlmeCcaConfirmCallback (PlmeCcaConfirmCallback c)
{
  NS_LOG_FUNCTION (this);
...
// TODO : CHECK
//void
//VlcPhy::SetPlmeEdConfirmCallback (PlmeEdConfirmCallback c)
//{
  //NS_LOG_FUNCTION (this);
...
void
VlcPhy::SetPlmeGetAttributeConfirmCallback (PlmeGetAttributeConfirmCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
VlcPhy::SetPlmeSetTRXStateConfirmCallback (PlmeSetTRXStateConfirmCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
VlcPhy::SetPlmeSetAttributeConfirmCallback (PlmeSetAttributeConfirmCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
VlcPhy::ChangeTrxState (VlcPhyEnumeration newState)
{
...
bool
VlcPhy::PhyIsBusy (void) const
{
  NS_LOG_FUNCTION (this << m_trxState);
  return ((m_trxState == IEEE_802_15_7_PHY_BUSY_TX)
          || (m_trxState == IEEE_802_15_7_PHY_BUSY_RX)
          || (m_trxState == IEEE_802_15_7_PHY_BUSY));
}
...
{
  NS_LOG_FUNCTION (this);
  VlcPhyEnumeration sensedChannelState = IEEE_802_15_7_PHY_UNSPECIFIED;
  ...
  if (PhyIsBusy ())
    {
      sensedChannelState = IEEE_802_15_7_PHY_BUSY;
    }
    ...
    { //sec 6.9.9 ED detection
      ...
      if (10 * log10 (m_ccaPeakPower / m_rxSensitivity) >= 10.0)
        ...
        {
          sensedChannelState = IEEE_802_15_7_PHY_IDLE;
        }
    }
...
void
VlcPhy::EndSetTRXState (void)
{
  NS_LOG_FUNCTION (this);

  NS_ABORT_IF ( (m_trxStatePending != IEEE_802_15_7_PHY_RX_ON) && (m_trxStatePending != IEEE_802_15_7_PHY_TX_ON) );
  ChangeTrxState (m_trxStatePending);
  m_trxStatePending = IEEE_802_15_7_PHY_IDLE;
...
VlcPhy::EndTx (void)
{
  ...
  NS_ABORT_IF ( (m_trxState != IEEE_802_15_7_PHY_BUSY_TX) && (m_trxState != IEEE_802_15_7_PHY_TRX_OFF));
  ...
  if (m_currentTxPacket.second == false)
    {
      ...
      if (!m_pdDataConfirmCallback.IsNull ())
        {
          m_pdDataConfirmCallback (IEEE_802_15_7_PHY_SUCCESS);
        }
    }
  else
    {
      ...
      if (!m_pdDataConfirmCallback.IsNull ())
        {
          ...
          NS_ASSERT (m_trxState ==  IEEE_802_15_7_PHY_TRX_OFF);
          m_pdDataConfirmCallback (m_trxState);
        }
    }
  m_currentTxPacket.first = 0;
...
Time
VlcPhy::CalculateTxTime (Ptr<const Packet> packet)
{
...
double
VlcPhy::GetDataOrSymbolRate (bool isData)
{
  NS_LOG_FUNCTION (this << isData);

  double rate = 0.0;

  NS_ASSERT (m_phyOption < IEEE_802_15_7_INVALID_PHY_OPTION);
...
Time
VlcPhy::GetPpduHeaderTxTime (void)
{
  NS_LOG_FUNCTION (this);
...
// TODO : CHECK
  ...
  totalPpduHdrSymbols = ppduHeaderSymbolNumbers[m_phyOption].shrPreamble\
    +ppduHeaderSymbolNumbers[m_phyOption].phrPhyHeader
    + ppduHeaderSymbolNumbers[m_phyOption].phrHcs;
...
void
VlcPhy::SetMyPhyOption (void)
{
  NS_LOG_FUNCTION (this);

  m_phyOption = IEEE_802_15_7_INVALID_PHY_OPTION;

  m_phyOption = IEEE_802_15_7_8B10B_OOK;
...
void
VlcPhy::SetTxPowerSpectralDensity (Ptr<SpectrumValue> txPsd)
{
...
//void
// VlcPhy::SetNoisePowerSpectralDensity (Ptr<const SpectrumValue> noisePsd)
// {
...
// VlcPhy::GetNoisePowerSpectralDensity (void)
// {
//   NS_LOG_FUNCTION (this);
//   return m_noise;
// }
...
void
VlcPhy::SetErrorModel (Ptr<VlcErrorModel> e)
{
...
Ptr<VlcErrorModel>
VlcPhy::GetErrorModel (void) const
{
  NS_LOG_FUNCTION (this);
  return m_errorModel;
}
...
uint64_t
VlcPhy::GetPhySHRDuration (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT (m_phyOption < IEEE_802_15_7_INVALID_PHY_OPTION);
...
double
VlcPhy::GetPhySymbolsPerOctet (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT (m_phyOption < IEEE_802_15_7_INVALID_PHY_OPTION);
...
int64_t
VlcPhy::AssignStreams (int64_t stream)
{
  NS_LOG_FUNCTION (this);
