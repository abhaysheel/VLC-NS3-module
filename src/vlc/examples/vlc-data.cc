...
/*
 * Try to send data end-to-end through a VlcMac <-> VlcPhy <->
 * SpectrumChannel <-> VlcPhy <-> VlcMac chain
 *
...
{
  NS_LOG_UNCOND ("VlcMcpsDataConfirmStatus = " << params.m_status);
}

static void StateChangeNotification (std::string context, Time now, VlcPhyEnumeration oldState, VlcPhyEnumeration newState)
{
  NS_LOG_UNCOND (context << " state change at " << now.GetSeconds ()
                         << " from " << VlcHelper::VlcPhyEnumerationPrinter (oldState)
                         << " to " << VlcHelper::VlcPhyEnumerationPrinter (newState));
}
...
int main (int argc, char *argv[])
{
  ...
  if (!extended)
    ...
    {
      Ptr<VlcMac> mac0 = dev0->GetMac();
      Ptr<VlcMac> mac1 = dev1->GetMac();
  ...
  sender1Mobility->SetPosition (Vector (0,0,0));   //Changed from 10 to 0m
  ...
  // Tracing
  vlcHelper.EnablePcapAll (std::string ("vlc-data"), true);
  AsciiTraceHelper ascii;
  Ptr<OutputStreamWrapper> stream = ascii.CreateFileStream ("vlc-data.tr");
  vlcHelper.EnableAsciiAll (stream);
